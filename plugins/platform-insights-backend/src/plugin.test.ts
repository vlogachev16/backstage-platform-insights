...
describe.skip('plugin', () =>  {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/platform-insights/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/platform-insights/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/platform-insights/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/platform-insights/todos/${createdTodoItem.id}`)
